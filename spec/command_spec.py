    ...
    with context("when it is created with CommandType STARTS_WITH"):
        with context("and a message starts with 'Set Theme '"):
            with it("should have a match"):
                ...
                expect(my_command.has_match(message_body="set theme Hello", user_security_level=SecurityLevel.ADMIN)).to(equal(True))
...
with describe("Given a command registry") as self:
    with context("when it is created"):
        ...
        with it("should have a bot name 'Dotty'"):
            command_registry = CommandRegistry()
            expect(" Dotty " in command_registry.get_commands_string(user_security_level=SecurityLevel.OWNER)).to(equal(True))
